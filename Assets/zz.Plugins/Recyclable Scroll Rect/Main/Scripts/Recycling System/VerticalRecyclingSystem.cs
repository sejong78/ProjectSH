...
namespace PolyAndCode.UI
{
    ...
    public class VerticalRecyclingSystem : RecyclingSystem
    {
        ...
        public void InitData( RectTransform prototypeCell, RectTransform viewport, RectTransform content, IRecyclableScrollRectDataSource dataSource, bool isGrid, int coloumns )
        {
		...
		public override IEnumerator InitCoroutine(System.Action onInitialized)
        {
			...
			float contentYSize = noOfRows * _cellHeight + (noOfRows - 1) * CellPadding;
            ...
            while ((poolSize < minPoolSize || currentPoolCoverage < requriedCoverage) && poolSize < DataSource.GetItemCount())
            {
                ...
                if (IsGrid)
                {
                    ...
                    if (++_bottomMostCellColoumn >= _coloumns)
                    {
						...
						posY -= _cellHeight + CellPadding;
						currentPoolCoverage += item.rect.height + CellPadding;
					}
                }
                else
                {
					item.anchoredPosition = new Vector2( 0, posY );
					posY = item.anchoredPosition.y - item.rect.height - CellPadding;
					currentPoolCoverage += item.rect.height + CellPadding;
				}
        ...
        private Vector2 RecycleTopToBottom()
        {
            ...
            while (_cellPool[topMostCellIndex].MinY() > _recyclableViewBounds.max.y && currentItemCount < DataSource.GetItemCount())
            {
                if (IsGrid)
                {
                    if (++_bottomMostCellColoumn >= _coloumns)
                    {
                        n++;
                        _bottomMostCellColoumn = 0;
                        posY = _cellPool[bottomMostCellIndex].anchoredPosition.y - _cellHeight - CellPadding;
                        additionalRows++;
                    }
                ...
                {
                    //Move top cell to bottom
                    posY = _cellPool[bottomMostCellIndex].anchoredPosition.y - _cellPool[bottomMostCellIndex].sizeDelta.y - CellPadding;
            ...
            if (IsGrid)
            {
                Content.sizeDelta += additionalRows * Vector2.up * ( _cellHeight + CellPadding );
			...
			float cellHeightWithPadding = _cellPool[topMostCellIndex].sizeDelta.y + CellPadding;
			_cellPool.ForEach( ( RectTransform cell ) => cell.anchoredPosition += n * Vector2.up * cellHeightWithPadding );
			Content.anchoredPosition -= n * Vector2.up * cellHeightWithPadding;
            _recycling = false;
            return -new Vector2(0, n * cellHeightWithPadding);
            ...
            while (_cellPool[bottomMostCellIndex].MaxY() < _recyclableViewBounds.min.y && currentItemCount > _cellPool.Count)
            {
                ...
                if (IsGrid)
                {
                    if (--_topMostCellColoumn < 0)
                    {
                        ...
                        posY = _cellPool[topMostCellIndex].anchoredPosition.y + _cellHeight + CellPadding;
                        additionalRows++;
                    }
                ...
                {
                    //Move bottom cell to top
                    posY = _cellPool[topMostCellIndex].anchoredPosition.y + _cellPool[topMostCellIndex].sizeDelta.y + CellPadding;
		...
		{
            int startIndex = topMostCellIndex;
            int cellCount = _cachedCells.SafeCount();
            int idx = 0;
            int dataIndex = Mathf.Max( 0, currentItemCount - _cachedCells.SafeCount() );

			for( int i = 0; i < cellCount; ++i )
            {
                idx = ( startIndex + i ) % cellCount;
                
				DataSource.SetCell( _cachedCells[ idx ], dataIndex + i );
			}
		}

		public override void GetCellOneByOne( Action<ICell,int> onGet )
		{
			int startIndex = topMostCellIndex;
			int cellCount = _cachedCells.SafeCount();
			int idx = 0;
			...
			for( int i = 0; i < cellCount; ++i )
			{
				...
				onGet.SafeExcute( _cachedCells[ idx ], dataIndex + i );
			}
		}
