...
namespace PolyAndCode.UI
{
    ...
    public class HorizontalRecyclingSystem : RecyclingSystem
    {
		...
		public void InitData( RectTransform prototypeCell, RectTransform viewport, RectTransform content, IRecyclableScrollRectDataSource dataSource, bool isGrid, int rows)
        {
        ...
        public override IEnumerator InitCoroutine(Action onInitialized)
        {
			...
			float contentXSize = coloums * _cellWidth + (coloums - 1) * CellPadding;
        ...
        private void CreateCellPool()
        {
            ...
            while ((poolSize < minPoolSize || currentPoolCoverage < requriedCoverage) && poolSize < DataSource.GetItemCount())
            {
                ...
                if (IsGrid)
                {
                    ...
                    {
						_RightMostCellRow = 0;
						posX += _cellWidth + CellPadding;
						currentPoolCoverage += item.rect.width + CellPadding;
					}
                }
                else
                {
					item.anchoredPosition = new Vector2( posX, 0 );
					posX = item.anchoredPosition.x + item.rect.width + CellPadding;
					currentPoolCoverage += item.rect.width + CellPadding;
				}
        ...
        private Vector2 RecycleLeftToRight()
        {
            ...
            while (_cellPool[leftMostCellIndex].MaxX() < _recyclableViewBounds.min.x && currentItemCount < DataSource.GetItemCount())
            {
                if (IsGrid)
                {
                    if (++_RightMostCellRow >= _rows)
                    {
                        n++;
                        _RightMostCellRow = 0;
                        posX = _cellPool[rightMostCellIndex].anchoredPosition.x + _cellWidth + CellPadding;
                        additionalColoums++;
                    }
                ...
                {
                    //Move Left most cell to right
                    posX = _cellPool[ rightMostCellIndex ].anchoredPosition.x + _cellPool[ rightMostCellIndex ].sizeDelta.x + CellPadding;
            ...
            if (IsGrid)
            {
                Content.sizeDelta += additionalColoums * Vector2.right * ( _cellWidth + CellPadding );
			...
			float cellWidthWithPadding = _cellPool[leftMostCellIndex].sizeDelta.x + CellPadding;
			_cellPool.ForEach( ( RectTransform cell ) => cell.anchoredPosition -= n * Vector2.right * cellWidthWithPadding );
			Content.anchoredPosition += n * Vector2.right * cellWidthWithPadding;
            _recycling = false;
            return n * Vector2.right * cellWidthWithPadding;
            ...
            while (_cellPool[rightMostCellIndex].MinX() > _recyclableViewBounds.max.x && currentItemCount > _cellPool.Count)
            {
                if (IsGrid)
                {
                    if (--_leftMostCellRow < 0)
                    {
                        ...
                        posX = _cellPool[leftMostCellIndex].anchoredPosition.x - _cellWidth - CellPadding;
                        additionalColoums++;
                    }
                ...
                {
                    //Move Right most cell to left
                    posX = _cellPool[leftMostCellIndex].anchoredPosition.x - _cellPool[leftMostCellIndex].sizeDelta.x - CellPadding;
		...
		{
			int startIndex = leftMostCellIndex;
